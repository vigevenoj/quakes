...
//	}
	...
	public void start() throws Exception {
		...
		WebTarget target = client.target(bridgeClientConfiguration.getApiURL() 
				+ "?access_token="
				+ bridgeClientConfiguration.getApiKey());
		...
		while (!eventInput.isClosed()) {
			...
			if (inboundEvent.getName() == "null") {
				...
				// TODO when this is working consistently, set log level back to debug
				logger.info(inboundEvent.getName());
			} else if (inboundEvent.getName() == "bridge data") {
				// Bridge Data:
				// This is a json object consisting of possible updates to bridge statuses
